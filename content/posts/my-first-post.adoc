...
ceci est un  lien https://docs.asciidoctor.org/asciidoctor/latest/install/[label]
...
> Java Flight Recorder (JFR) is a tool for collecting diagnostic and profiling data about a running Java application. It is integrated into the Java Virtual Machine (JVM) and causes almost no 
...
IMPORTANT: Admonitions can also encapsulate any block content, which we’ll cover later. 

WARNING: Admonitions can also encapsulate any block content, which we’ll cover later. 

CAUTION: Admonitions can also encapsulate any block content, which we’ll cover later. 
